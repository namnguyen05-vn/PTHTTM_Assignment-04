...
def analyze(require_complete=True):
    ...
    d100=load_data('cifar100')
    train_hashes={hashlib.sha256(im.tobytes()).digest() for im in d100['x']}
    mask=np.array([hashlib.sha256(im.tobytes()).digest() not in train_hashes for im in d100['x_test']])
    sensitivity=[]
    for _,m in df[df.dataset=='cifar100'].iterrows():
        folder=ROOT/'results'/f'cifar100_{m.backend}_{m.variant}'
        with np.load(folder/'test_outputs.npz') as a:
            acc=float((a['logits'][mask].argmax(1)==a['labels'][mask]).mean())
        sensitivity.append(dict(backend=m.backend,variant=m.variant,official_accuracy=m.accuracy,unseen_pixel_accuracy=acc,retained_test_images=int(mask.sum()),excluded_test_images=int((~mask).sum())))
    (ROOT/'results'/'cifar100_duplicate_sensitivity.json').write_text(json.dumps(sensitivity,indent=2))
    ...
    if len(df)==18:
        ...
        pilot_records=[]
        for ax,name in zip(axes,DATASETS):
            for label,folder,style in [('Pilot: no Dense BN',ROOT/'experiments/pilot_v1/results'/f'{name}_tensorflow_improved','--'),('Final: with Dense BN',ROOT/'results'/f'{name}_tensorflow_improved','-')]:
                h=pd.read_csv(folder/'history.csv');ax.plot(h.epoch,h.val_loss,style,label=label)
                pilot_records.append(dict(dataset=name,architecture=label,best_val_loss=float(h.val_loss.min()),final_train_loss=float(h.train_loss.iloc[-1])))
            ax.set(xlabel='Epoch',ylabel='Validation loss',title=name.upper());ax.legend(fontsize=7);ax.grid(alpha=.2)
        fig.tight_layout();save(fig,'pilot_dense_bn.png')
        pd.DataFrame(pilot_records).to_csv(ROOT/'results'/'pilot_comparison.csv',index=False)
    from .numpy_cnn import CNN
    mnist=load_data('mnist');net=CNN(1,28,10)
    with np.load(ROOT/'results/mnist_numpy_baseline/weights.npz') as weights:net.load_state(dict(weights))
    sample=mnist['x_test'][:1].astype('float32')/255
    fmap=np.maximum(net.layers[0].forward(sample,False),0)
    fig,axes=plt.subplots(1,9,figsize=(11,2));axes[0].imshow(sample[0,0],cmap='gray');axes[0].set_title('Input')
    for i in range(8):axes[i+1].imshow(fmap[0,i],cmap='magma');axes[i+1].set_title('Map '+str(i+1))
    for ax in axes:ax.axis('off')
    fig.tight_layout();save(fig,'mnist_feature_maps.png')
