...
def predict(backend,dataset,variant,images):
    """images: NCHW uint8 pixels, or float32 already divided by 255. Uses CPU."""
    cfg=DATASETS[dataset];folder=ROOT/'results'/f'{dataset}_{backend}_{variant}'
    ...
    if backend=='pytorch':
        ...
        model=TorchCNN(cfg['channels'],cfg['size'],cfg['classes'],variant)
...
def main():
    ...
    p.add_argument('--test-id',type=int,default=0);p.add_argument('--verify-checkpoint',action='store_true');a=p.parse_args()
    ...
    logits=predict(a.backend,a.dataset,a.variant,d['x_test'][ids])
    if a.verify_checkpoint:
        with np.load(ROOT/'results'/f'{a.dataset}_{a.backend}_{a.variant}'/'test_outputs.npz') as saved:reference=saved['logits'][ids]
        np.testing.assert_allclose(logits,reference,atol=3e-4,rtol=3e-4)
        error=float(np.max(np.abs(logits-reference)))
        print(json.dumps(dict(dataset=a.dataset,backend=a.backend,variant=a.variant,checkpoint_verified=True,samples=len(ids),max_abs_error=error)))
    else:
